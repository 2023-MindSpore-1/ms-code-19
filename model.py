...
class CMPNet(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, timeslot, feature, poi, device):
        ...
        for num in range(self.timeslot):
            layers.append(AdaCNN(self.in_channel, self.out_channel, self.kernel_size, self.feature, device))
        self.layers = nn.ModuleList(layers)
        self.fea_comp = AuxCMP(self.poi, self.device)
    ...
    def _ini_hidden(self, bs, c, w, h):
        # print(bs, c, w, h)
        return mindspore.ops.zeros((bs, c, w, h)).to(self.device)


##################### super resolution network #####################
class SRNet_cell(nn.Module):
    def __init__(self, in_channel, channel_n, scaler_n, resnet_n, step, weather=None):
        super(SRNet_cell, self).__init__()
        self.step = step
        self.channel_n = channel_n
        self.weight = mindspore.Tensor([0.1, 0.3, 0.6])
        self.weight = Tensor(self.weight ,mindspore.float32)
        # ResExt
    ...
    def forward(self, x, feature_reshape=None):
        if feature_reshape is None:
            ...
            for s in range(self.step):
                if s == 0:
                    x0 = mindspore.ops.cat((x, feature_reshape), dim=1)
                    x0 = self.ch1(x0)
                x1 = self.exnet(x0)
                x2 = self.upnet(x1)
                outputs += self.weight[s] * self.ch2(x2)
...
class SRNet(nn.Module):
    def __init__(self, in_channel, channel_n, scaler_n, resnet_n, step, side):
        super(SRNet, self).__init__()
        self.block_n = int(scaler_n / 2)
        self.extf = FeaExtraction(side)
